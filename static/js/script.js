...
$(function(){
   ...
   //
   //
   // remove partner / decline invite
   $('.cancelCouple').live('click',function(event){
      event.preventDefault();
      var coupleContainer = $(this).closest('.coupleContainer');
      var action,parentDiv = $(this).parent();
      var inviteCode = parentDiv.attr("data-inviteCode");
      var partnerKey = parentDiv.attr("data-partnerkey")
      var errorPar = $('.inviteCoupleError')
      ...
      var url = "/json/handleInvite";
      if(action=="removePartner"){
          url = "/json/removePartner";
      }
      log(inviteCode);
      log(action);
      log(url);
      ...
      $.getJSON(url, data, function(data) {
        log(data);
        if(data.success){
            coupleContainer.hide('fast',function(){
               $(this).remove();
               // if no other invites or whatnot, show the form fields
               if(!$('.coupleContainer').length){
                $('#inviteFields').tmpl(data).insertBefore('.inviteCoupleError');
   ...
   $('.coupleAccept').live('click',function(event){
      event.preventDefault();
      var coupleContainer = $(this).closest('.coupleContainer');
      ...
      $.getJSON('/json/handleInvite', data, function(data) {
        ...
        if(data.success){
            coupleContainer.hide('fast',function(){
               $(this).remove();
               // now insert the new couple record
               $('#newCouple').tmpl(data).insertBefore('.inviteCoupleError');
            });
        }else if(!data.success && data.reason){
            errorPar.text(data.reason);
            inputContainer.addClass('error');
   ...
   if((!$('#inviteCouple').exists())&&($('#main').hasClass('home'))){
